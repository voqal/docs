...
Currently, Voqal is only available as a plugin for JetBrains IDEs. Adding a standalone CLI would allow developers to
create and customize Voqal Assistants for any task they need. For example, a developer could create a Voqal Assistant
which is only able to query Google Analytics, deploy it to a server, and serve it as an internal API for their team
to use.
...
automatically by Voqal if it were in a "Tool Mode". This would allow developers to create Voqal Tools more quickly as
they could simply switch modes, have a conversation about a tool they want to create, loop through some trial and error
to get the tool working, and then switch back to normal mode with a new tool available.

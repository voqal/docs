...
The Voqal Assistant comes with five embedded modes: `Code`, `Dictate`, `Edit`, `Focus`, and `Idle`.
Each mode represents a different state Voqal can be in and are designed to handle different types of tasks. Each mode
has access to different [Voqal Tools](/tools/overview), so it's important to be in the "right gear" for the task at hand.
You won't be able to edit text in the focus mode.

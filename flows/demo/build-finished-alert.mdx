...
function:
  ...
  exec:
    command: Start-Sleep -Seconds 10; while ($true) { if (-not (gh run list --status=in_progress --json status | ConvertFrom-Json | Where-Object { $_.status -eq "in_progress" })) { break } Start-Sleep -Seconds 10 }
    type: powershell
```
````